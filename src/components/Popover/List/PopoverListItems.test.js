...
describe("PopoverListItems.js Test Suite", () => {
  beforeEach(() => {
    render(
      <>
        <p
          data-testid="opener-wrapper"
          onClick={(e) =>
            popoverListItemsRef.current.showAt(e.detail.targetRef)
          }
        >
          Open Popover
        </p>
        <PopoverListItems
          popoverRef={popoverListItemsRef}
          title="Some Title"
          items={popoverItems}
        />
      </>
  ...
  test("should render two items in the list if passed correctly", async () => {
    ...
    const popoverListItems = screen.getByTestId("popoverListItems-wrapper");
    const popoverListItemsListOfElements = screen.getByTestId(
      "popoverListItems-listOfElements-wrapper"
    );
    expect(popoverListItems).toBeInTheDocument();
